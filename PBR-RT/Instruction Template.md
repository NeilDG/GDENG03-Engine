...
- Check other related MD files in the project to get a better context.
- Perform a feedback loop. Run the engine for X seconds, where X depends on the context of the task. Check the PBR-RT/anito-debug directory for the logs and frames created and use these data to verify if the task was completed. Repeat and iterate on the coding process, until the task at hand is satisfied.
- Be proactive in profiling. Modify the profilers as necessary, based on the task at hand. Perform the same feedback loop.
