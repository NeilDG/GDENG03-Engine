...
- **Engine Name Prefix**: All major modules, classes, and outputs must use the **Anito** prefix (e.g., `AnitoRenderer`, `AnitoECS`, `AnitoTerrain`).
- **Targets**: PC (high-end Vulkan/DirectX) + Android (Vulkan-first). Cross-platform from day one.
...
Constraints: AAA fidelity, PC+Android targets, Anito prefix, high performance.
Output required: [code / design / benchmark / etc.]

2. **Collaboration Flow**: Coordinator → Specialist Agents → Integrator & Tester → Reviewer loop.

3. **Versioning**: All code must be committed to the DLSU-GAME-Lab Git repository with clear Anito-prefixed branches.
...
**Version:** 2.0 (with detailed prompts and orchestration guide)
